@if(!empty($data))
<form action="{{url('update-requests/'.$data->id)}}" method="post">
	@csrf
	...
	<div class="form-group">
		<label for="text">Request sevice:</label>
		<select class="form-control"  name="service" required="" >
			<option value="">Select Service</option>
			@if(count($category) > 0)
			@foreach($category as $rows)
			<option value="{{$rows->category_id}}" @if($data->service == $rows->category_id) {{'selected'}} @endif>{{$rows->category_name}}</option>
			@endforeach
			@endif
		</select>
	</div>
